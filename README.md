...
You did all the css and the client tells you that he wants this block to be also used on another page, but it should be 2 times bigger.
If you used pixels, than you need to create modifier for this block and override all sizes.
...
Namespacing makes code readable and clear. And they're also are the lifesavers when you have to redesign old project with clattered global namespace.
E.g. once you add button or modal to the page you are redesigning, they instantly get old styles you don't need. Adding "c-" prefix to them saves the situation.
...
If this list needs to look differently in another page or block, just change the layout. No need for styles overriding.

## Use helpers and modifiers
Imagine you have similar looking forms with different contents used allover the site.
Most forms have button, but each time the button background/size/positioning is a bit different for design needs.
You can use BEM mix and add "c-signup-form__btn" to style the button, but in this case it is better not to tie button modifications to particular form, but create modifiers and helpers to that can be reused when needed. BEM mixes are good when the styles are component specific, and won't be reused in other components.
Look at this button:
...
I took this approaches from BEM and Atomic CSS methodologies.
...
For example you have site.less or whatever where you put all your components styles. It'll become bigger with time and you're gonna have to use search tool for finding component you need.
Searching will be easier if you move each component style into separate file like "c-button", "c-product", etc. Splitting mixins, helpers and variables files would also be usable.
